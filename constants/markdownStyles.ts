...
export function fenceColors(theme: Theme) {
  return {
    bg:     theme === 'light' ? '#f6f8fa' : '#0d0d12',
    text:   theme === 'light' ? '#1f2328' : '#cdd9e5',
    border: theme === 'light' ? '#e2e2e8' : '#2a2a38',
  };
}
...
export function markdownStyles(theme: Theme) {
  ...
  return {
    ...
    th:                   { backgroundColor: theme === 'light' ? '#f0effe' : '#1e1a40', fontWeight: '600' as const, paddingHorizontal: 10, paddingVertical: 6, borderRightWidth: 1, borderBottomWidth: 1, borderColor: c.border },
    td:                   { paddingHorizontal: 10, paddingVertical: 6, borderRightWidth: 1, borderBottomWidth: 1, borderColor: c.border },
